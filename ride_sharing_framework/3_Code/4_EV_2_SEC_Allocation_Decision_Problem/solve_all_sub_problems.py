...
# ------------------------------------------
# FUNCTION 01 - solve_sub_problems
# ------------------------------------------
def solve_sub_problems(input_folder,
                       output_folder,
                       solution_folder
                      ):
    ...
    # 5. We collect all the configurations to be analysed
    list_of_folders = os.listdir(input_folder)
    if (".DS_Store") in list_of_folders:
        list_of_folders.remove(".DS_Store")
    list_of_folders.sort()

    # 6. We run the set of instances for each configuration
    for folder in list_of_folders:
        # 6.1. We create the output folder to create the instances
        os.mkdir(output_folder + folder)

        # 6.2. We collect the list of instances to be solved (in sorted order) under the desired configuration
        list_of_files = os.listdir(input_folder + folder)
        if (".DS_Store") in list_of_files:
            list_of_files.remove(".DS_Store")
        list_of_files.sort()

        # 6.3. We traverse the instances
        for file in list_of_files:
            # 6.3.1. We get the name of the input and output files
...
if __name__ == '__main__':
    ...
    input_folder = "../../2_Instances/Analysis/"
    output_folder = "../../4_Solutions/Analysis/"
    ...
    solve_sub_problems(input_folder,
                       output_folder,
                       solution_folder
                      )
