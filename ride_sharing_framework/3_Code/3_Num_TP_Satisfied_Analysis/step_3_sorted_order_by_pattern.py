...
def sort_instance(input_file,
    ...
    content = list(filter(lambda x: pattern in x, content))
...
def create_folder(input_folder,
    ...
    for file in list_of_files:
        ...
        output_file_name = output_folder + file[:-4] + "_" + pattern + file[-4:]

        # 4.2. We solve the instance
        sort_instance(input_file_name,
                      output_file_name,
                      pattern
                     )
...
def my_main(input_folder,
    ...
    # 1. We traverse the different patterns in the list
    index = 1
    for pattern in patterns_list:
        # 1.1. We create the output folder name
        new_output_folder = output_folder + "3_" + str(index).zfill(2) + "_" + pattern + "/"

        # 1.2. We call to the function create_folder
        create_folder(input_folder,
                      new_output_folder,
                      pattern
                      )

        # 1.3. We increase the index
        index += 1
